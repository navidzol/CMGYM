'use client';

import { useEffect, useState, useCallback, useRef } from 'react';
import { api, requireAuth } from '../../lib/api';

interface SessionSet {
  id: string;
  exercise_id: string;
  exercise_name: string;
  exercise_type: string;
  set_number: number;
  reps: number | null;
  weight_kg: number | null;
  rpe: number | null;
  duration_s: number | null;
  distance_m: number | null;
  completed_at: string | null;
}

interface ScheduleExercise {
  exercise_id: string;
  exercise_name: string;
  type: string;
  sets: number;
  reps: number;
  rest_s: number;
}

export default function WorkoutPage() {
  const [sessionId, setSessionId] = useState<string | null>(null);
  const [session, setSession] = useState<any>(null);
  const [sets, setSets] = useState<SessionSet[]>([]);
  const [schedule, setSchedule] = useState<ScheduleExercise[]>([]);
  const [loading, setLoading] = useState(true);
  const [msg, setMsg] = useState('');
  const [prAlert, setPrAlert] = useState('');

  // Current set form
  const [currentExercise, setCurrentExercise] = useState<ScheduleExercise | null>(null);
  const [reps, setReps] = useState(10);
  const [weight, setWeight] = useState(0);
  const [rpe, setRpe] = useState<number | undefined>(undefined);

  // Rest timer
  const [restTime, setRestTime] = useState(0);
  const [restTarget, setRestTarget] = useState(90);
  const [timerActive, setTimerActive] = useState(false);
  const timerRef = useRef<NodeJS.Timeout | null>(null);

  useEffect(() => {
    requireAuth();
    const params = new URLSearchParams(window.location.search);
    const id = params.get('session');
    if (id) {
      setSessionId(id);
      loadSession(id);
    } else {
      setLoading(false);
      setMsg('No session ID provided. Go to Programme to start a workout.');
    }
  }, []);

  // Timer
  useEffect(() => {
    if (timerActive) {
      timerRef.current = setInterval(() => {
        setRestTime(t => t + 1);
      }, 1000);
    } else if (timerRef.current) {
      clearInterval(timerRef.current);
    }
    return () => { if (timerRef.current) clearInterval(timerRef.current); };
  }, [timerActive]);

  async function loadSession(id: string) {
    try {
      const res = await api<any>(`/sessions/${id}`);
      setSession(res);

      // Load existing sets
      const setsRes = await api<SessionSet[]>(`/sessions/${id}/sets`);
      setSets(Array.isArray(setsRes) ? setsRes : []);

      // Parse schedule from generated session
      if (res.schedule_json) {
        const sched = typeof res.schedule_json === 'string' ? JSON.parse(res.schedule_json) : res.schedule_json;
        const exercises = sched.exercises || [];
        if (sched.cardio) exercises.push(sched.cardio);
        setSchedule(exercises);
        if (exercises.length > 0 && !currentExercise) {
          setCurrentExercise(exercises[0]);
          setReps(exercises[0].reps || 10);
          setRestTarget(exercises[0].rest_s || 90);
        }
      }
    } catch (e: any) {
      setMsg(e.message);
    } finally {
      setLoading(false);
    }
  }

  async function logSet() {
    if (!sessionId || !currentExercise) return;
    const setsForExercise = sets.filter(s => s.exercise_id === currentExercise.exercise_id);
    const setNumber = setsForExercise.length + 1;

    try {
      const body: any = {
        exercise_id: currentExercise.exercise_id,
        set_number: setNumber,
        reps,
      };
      if (currentExercise.type === 'strength') body.weight_kg = weight;
      if (rpe) body.rpe = rpe;

      const res = await api<any>(`/sessions/${sessionId}/sets`, { method: 'POST', body });
      setSets([...sets, res]);

      if (res.pr_hit) {
        setPrAlert(`New PR! Est. 1RM: ${res.estimated_1rm?.toFixed(1)} kg`);
        setTimeout(() => setPrAlert(''), 5000);
      }

      // Start rest timer
      setRestTime(0);
      setTimerActive(true);
    } catch (e: any) {
      setMsg(e.message);
    }
  }

  async function finishWorkout() {
    if (!sessionId) return;
    try {
      await api(`/sessions/${sessionId}`, { method: 'PATCH', body: {} });
      setMsg('Workout complete!');
      setTimeout(() => { window.location.href = '/dashboard'; }, 1500);
    } catch (e: any) {
      setMsg(e.message);
    }
  }

  function selectExercise(ex: ScheduleExercise) {
    setCurrentExercise(ex);
    setReps(ex.reps || 10);
    setRestTarget(ex.rest_s || 90);
    setTimerActive(false);
    setRestTime(0);
  }

  if (loading) return <Main><p style={{ color: '#6B6B8A' }}>Loading workout...</p></Main>;
  if (!sessionId) return <Main><p style={{ color: '#6B6B8A' }}>{msg}</p></Main>;

  const completedExercises = new Set(sets.map(s => s.exercise_id));

  return (
    <Main>
      {prAlert && (
        <div style={{ backgroundColor: '#5B4FE8', color: '#fff', padding: '1rem', borderRadius: '12px', marginBottom: '1rem', textAlign: 'center', fontWeight: 700 }}>
          {prAlert}
        </div>
      )}
      {msg && <p style={{ color: '#F97316', fontSize: '0.875rem', marginBottom: '1rem' }}>{msg}</p>}

      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        <h1 style={{ fontSize: '1.3rem', fontWeight: 700 }}>Workout</h1>
        <button onClick={finishWorkout} style={{ ...btnStyle, backgroundColor: '#4FE8A8', color: '#0d0d1a', fontWeight: 600 }}>
          Finish Workout
        </button>
      </div>

      <div style={{ display: 'flex', gap: '1.5rem', flexWrap: 'wrap' }}>
        {/* Exercise List */}
        <div style={{ flex: '1', minWidth: '200px' }}>
          <h2 style={{ color: '#9B9BB0', fontSize: '0.875rem', marginBottom: '0.5rem' }}>Exercises</h2>
          {schedule.map((ex, i) => {
            const setsForEx = sets.filter(s => s.exercise_id === ex.exercise_id);
            const isActive = currentExercise?.exercise_id === ex.exercise_id;
            return (
              <div key={i} onClick={() => selectExercise(ex)}
                style={{
                  ...cardStyle,
                  marginBottom: '0.5rem',
                  cursor: 'pointer',
                  border: isActive ? '1px solid #5B4FE8' : '1px solid #2D2D44',
                  opacity: setsForEx.length >= ex.sets ? 0.5 : 1,
                }}>
                <p style={{ fontWeight: 600, fontSize: '0.875rem' }}>{ex.exercise_name}</p>
                <p style={{ color: '#6B6B8A', fontSize: '0.75rem' }}>
                  {setsForEx.length}/{ex.sets} sets &middot; {ex.type}
                </p>
              </div>
            );
          })}
        </div>

        {/* Logging Area */}
        <div style={{ flex: '2', minWidth: '300px' }}>
          {/* Rest Timer */}
          {timerActive && (
            <div style={{
              ...cardStyle,
              marginBottom: '1rem',
              textAlign: 'center',
              borderColor: restTime >= restTarget ? '#4FE8A8' : '#2D2D44',
            }}>
              <p style={{ color: '#6B6B8A', fontSize: '0.75rem' }}>REST</p>
              <p style={{ fontSize: '2rem', fontWeight: 700, fontFamily: 'monospace', color: restTime >= restTarget ? '#4FE8A8' : '#fff' }}>
                {Math.floor(restTime / 60)}:{String(restTime % 60).padStart(2, '0')}
              </p>
              <p style={{ color: '#6B6B8A', fontSize: '0.75rem' }}>Target: {restTarget}s</p>
              <button onClick={() => { setTimerActive(false); setRestTime(0); }}
                style={{ ...btnStyle, marginTop: '0.5rem', fontSize: '0.75rem' }}>Skip Rest</button>
            </div>
          )}

          {currentExercise && (
            <div style={cardStyle}>
              <h2 style={{ fontWeight: 600, marginBottom: '0.75rem' }}>{currentExercise.exercise_name}</h2>
              <p style={{ color: '#6B6B8A', fontSize: '0.875rem', marginBottom: '1rem' }}>
                Target: {currentExercise.sets} sets x {currentExercise.reps} reps
              </p>

              {/* Previous sets for this exercise */}
              {sets.filter(s => s.exercise_id === currentExercise.exercise_id).length > 0 && (
                <div style={{ marginBottom: '1rem' }}>
                  <p style={{ color: '#6B6B8A', fontSize: '0.75rem', marginBottom: '0.25rem' }}>Logged sets:</p>
                  {sets.filter(s => s.exercise_id === currentExercise.exercise_id).map(s => (
                    <p key={s.id} style={{ fontSize: '0.8rem', color: '#9B9BB0' }}>
                      Set {s.set_number}: {s.reps} reps {s.weight_kg ? `@ ${s.weight_kg}kg` : ''} {s.rpe ? `RPE ${s.rpe}` : ''}
                    </p>
                  ))}
                </div>
              )}

              {/* Log form */}
              <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap', marginBottom: '1rem' }}>
                <label style={labelStyle}>
                  Reps
                  <input type="number" min={0} value={reps} onChange={e => setReps(+e.target.value)} style={{ ...inputStyle, width: '80px' }} />
                </label>
                {currentExercise.type === 'strength' && (
                  <label style={labelStyle}>
                    Weight (kg)
                    <input type="number" min={0} step={0.5} value={weight} onChange={e => setWeight(+e.target.value)} style={{ ...inputStyle, width: '100px' }} />
                  </label>
                )}
                <label style={labelStyle}>
                  RPE (1-10)
                  <input type="number" min={1} max={10} value={rpe || ''} onChange={e => setRpe(e.target.value ? +e.target.value : undefined)} style={{ ...inputStyle, width: '80px' }} />
                </label>
              </div>

              <button onClick={logSet} style={{ ...btnStyle, backgroundColor: '#5B4FE8', color: '#fff', width: '100%', padding: '0.75rem' }}>
                Log Set {sets.filter(s => s.exercise_id === currentExercise.exercise_id).length + 1}
              </button>
            </div>
          )}

          {!currentExercise && schedule.length === 0 && (
            <div style={cardStyle}>
              <p style={{ color: '#6B6B8A' }}>No exercises in this session. This may happen if your exercise pool was empty when the programme was generated.</p>
            </div>
          )}
        </div>
      </div>
    </Main>
  );
}

function Main({ children }: { children: React.ReactNode }) {
  return <main style={{ padding: '2rem', maxWidth: '900px', margin: '0 auto' }}>{children}</main>;
}

const inputStyle: React.CSSProperties = {
  padding: '0.625rem',
  backgroundColor: '#0d0d1a',
  color: '#fff',
  border: '1px solid #2D2D44',
  borderRadius: '8px',
  fontSize: '1rem',
  outline: 'none',
  marginTop: '0.25rem',
};

const btnStyle: React.CSSProperties = {
  padding: '0.5rem 1rem',
  border: '1px solid #2D2D44',
  borderRadius: '8px',
  cursor: 'pointer',
  fontSize: '0.875rem',
  backgroundColor: '#1A1A2E',
  color: '#9B9BB0',
};

const cardStyle: React.CSSProperties = {
  backgroundColor: '#1A1A2E',
  borderRadius: '12px',
  border: '1px solid #2D2D44',
  padding: '1rem',
};

const labelStyle: React.CSSProperties = {
  color: '#9B9BB0',
  fontSize: '0.875rem',
};
