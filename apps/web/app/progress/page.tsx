...
interface PR { id: string; exercise_name: string; metric: string; value: number; achieved_at: string; }
interface Report {
  type: string; date?: string; start_date?: string; end_date?: string;
  total_volume_kg: number; sessions_completed: number; planned_per_week?: number; cardio_minutes?: number; prs?: PR[];
}
...
export default function ProgressPage() {
  ...
  useEffect(() => { requireAuth(); loadPRs(); }, []);

  async function loadPRs() {
    setLoading(true);
    try { const res = await api<PR[]>('/records'); setPrs(Array.isArray(res) ? res : []); }
    catch { setPrs([]); }
    setLoading(false);
  }
  ...
  async function loadReport() {
    setMsg('');
    try { const res = await api<Report>(`/reports?type=${reportType}&date=${reportDate}`); setReport(res); }
  ...
  return (
    <Main>
      ...
      {tab === 'prs' && (
        loading ? <p style={{ color: 'var(--text-muted)' }}>Loading...</p> : prs.length === 0 ? (
          <p style={{ color: 'var(--text-muted)' }}>No personal records yet. PRs are automatically detected when you log sets.</p>
        ) : (
            ...
            {prs.map(pr => (
              <div key={pr.id} style={cardStyle}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <p style={{ color: 'var(--accent)', fontWeight: 700, fontSize: '1.25rem' }}>{pr.value.toFixed(1)} kg</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <>
          <div style={{ ...cardStyle, marginBottom: '1rem' }}>
            <div style={{ display: 'flex', gap: '1rem', alignItems: 'flex-end', flexWrap: 'wrap' }}>
              <label style={labelStyle}>Period<select value={reportType} onChange={e => setReportType(e.target.value)} style={inputStyle}>
                <option value="daily">Daily</option><option value="weekly">Weekly</option><option value="monthly">Monthly</option>
              </select></label>
              <label style={labelStyle}>Date<input type="date" value={reportDate} onChange={e => setReportDate(e.target.value)} style={inputStyle} /></label>
              <button onClick={loadReport} style={{ ...btnStyle, backgroundColor: 'var(--accent)', color: '#fff' }}>Generate Report</button>
            </div>
          </div>
          ...
          {report && (
            <div style={cardStyle}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '1rem' }}>
                ...
                {report.planned_per_week !== undefined && <StatCard label="Planned/Week" value={`${report.planned_per_week}`} />}
                {report.cardio_minutes !== undefined && <StatCard label="Cardio" value={`${(report.cardio_minutes || 0).toFixed(0)} min`} />}
              </div>
              {report.prs && report.prs.length > 0 && (
                <div style={{ marginTop: '1rem' }}>
                  <p style={{ color: 'var(--text-secondary)', fontSize: '0.875rem', marginBottom: '0.5rem' }}>PRs in this period:</p>
                  {report.prs.map((pr, i) => (
                    <p key={i} style={{ fontSize: '0.8rem', color: 'var(--accent)' }}>{pr.exercise_name}: {pr.value.toFixed(1)} kg</p>
                  ))}
                </div>
              )}
            </div>
...
const inputStyle: React.CSSProperties = {
  ...
  border: '1px solid var(--border)', borderRadius: '8px', fontSize: '0.875rem', outline: 'none', marginTop: '0.25rem',
};
