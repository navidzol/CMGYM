...
interface Family { id: string; name: string; invite_code: string; invite_expires_at: string | null; created_at: string; members?: Member[]; }
...
export default function FamilyPage() {
  ...
  useEffect(() => { requireAuth(); setLoading(false); }, []);
  ...
  async function createFamily() {
    ...
    try {
      ...
      setFamilyName(''); setSelectedFamily(res); setFamilies([...families, res]); setTab('view');
    } catch (e: any) { setMsg(e.message); }
  }
  ...
  async function joinFamily() {
    ...
    try {
      ...
      setMsg('Joined family successfully!'); setTab('view'); await loadFamily(joinFamilyId);
    } catch (e: any) { setMsg(e.message); }
  }
    ...
    <Main>
        ...
        selectedFamily ? (
          <div>
            ...
            <div style={{ ...cardStyle, marginBottom: '1rem' }}>
              <h3 style={{ color: 'var(--text-secondary)', fontSize: '0.875rem', marginBottom: '0.5rem' }}>Members</h3>
              {selectedFamily.members?.map(m => (
                <div key={m.user_id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.5rem 0', borderBottom: '1px solid var(--border)' }}>
                  <div>
                    <span style={{ fontWeight: 600 }}>{m.display_name}</span>
                    <span style={{ color: 'var(--text-muted)', fontSize: '0.75rem', marginLeft: '0.5rem' }}>{m.role}</span>
                  </div>
                  {m.role !== 'admin' && (
                    <button onClick={() => removeMember(selectedFamily.id, m.user_id)}
                      style={{ ...btnStyle, color: 'var(--warning)', fontSize: '0.7rem', padding: '0.25rem 0.5rem' }}>Remove</button>
                  )}
                </div>
              ))}
            </div>
            ...
            <div style={{ ...cardStyle, marginBottom: '1rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
                <h3 style={{ color: 'var(--text-secondary)', fontSize: '0.875rem' }}>Leaderboard</h3>
              ...
              {leaderboard.length === 0 ? (
                <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>No data yet. Start logging workouts!</p>
                ...
                <div key={entry.user_id} style={{ display: 'flex', justifyContent: 'space-between', padding: '0.5rem 0', borderBottom: '1px solid var(--border)' }}>
                  ...
                  <span style={{ color: 'var(--accent)', fontWeight: 600 }}>{(entry.total_volume || 0).toFixed(0)} kg &middot; {entry.session_days} days</span>
                </div>
              ))}
            </div>

            <div style={cardStyle}>
              <h3 style={{ color: 'var(--text-secondary)', fontSize: '0.875rem', marginBottom: '0.5rem' }}>Recent Activity</h3>
              {ledger.length === 0 ? (
                ...
                <div key={i} style={{ padding: '0.375rem 0', borderBottom: '1px solid var(--border)', fontSize: '0.8rem' }}>
                  ...
                  <span style={{ color: 'var(--text-muted)', fontSize: '0.7rem' }}>{new Date(entry.logged_at).toLocaleString()}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={cardStyle}>
            <p style={{ color: 'var(--text-muted)' }}>No family selected. Create a new family or join one with an invite code.</p>
            <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem', marginTop: '0.5rem' }}>If you already have a family ID, enter it below to view it.</p>
            <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1rem' }}>
              ...
              <button onClick={() => joinFamilyId && loadFamily(joinFamilyId)}
                style={{ ...btnStyle, backgroundColor: 'var(--accent)', color: '#fff' }}>View</button>
            </div>
          </div>
        )
      )}
      ...
      {tab === 'create' && (
        <div style={cardStyle}>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem', marginBottom: '1rem' }}>Create a family to train together and compete on leaderboards.</p>
          <label style={labelStyle}>Family Name
            <input value={familyName} onChange={e => setFamilyName(e.target.value)} placeholder="e.g. The Swole Squad" style={inputStyle} />
          </label>
          <button onClick={createFamily} style={{ ...btnStyle, backgroundColor: 'var(--accent)', color: '#fff', marginTop: '1rem' }}>Create Family</button>
        </div>
      )}
      ...
      {tab === 'join' && (
        <div style={cardStyle}>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem', marginBottom: '1rem' }}>Enter the family ID and invite code shared by a family admin.</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <label style={labelStyle}>Family ID<input value={joinFamilyId} onChange={e => setJoinFamilyId(e.target.value)} placeholder="UUID" style={inputStyle} /></label>
            <label style={labelStyle}>Invite Code<input value={inviteCode} onChange={e => setInviteCode(e.target.value)} placeholder="8-character code" style={inputStyle} /></label>
            <button onClick={joinFamily} style={{ ...btnStyle, backgroundColor: 'var(--accent)', color: '#fff' }}>Join Family</button>
          </div>
        </div>
      )}
    </Main>
