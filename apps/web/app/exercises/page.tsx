...
export default function ExercisesPage() {
  ...
  async function loadPool() {
    setLoading(true);
    try {
      const res = await api<PoolEntry[]>('/pools');
      setPool(Array.isArray(res) ? res : []);
    } catch (e: any) {
      setMsg(e.message);
    } finally {
      setLoading(false);
  ...
  async function addToPool(ex: Exercise) {
    try {
      await api('/pools', {
        method: 'POST',
        body: { exercise_id: ex.id, muscle_family_id: ex.muscle_family_id || null },
      });
  ...
  return (
    <Main>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.5rem' }}>
        {(['browse', 'pool', 'seed'] as const).map(t => (
          <button key={t} onClick={() => { setTab(t); setMsg(''); }}
            style={{
              ...btnStyle,
              backgroundColor: tab === t ? '#5B4FE8' : '#1A1A2E',
      ...
      {tab === 'browse' && (
        <>
          <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem', flexWrap: 'wrap' }}>
            ...
            <button onClick={() => searchExercises()} style={{ ...btnStyle, backgroundColor: '#5B4FE8', color: '#fff' }}>Search</button>
            <button onClick={fetchExternal} style={{ ...btnStyle, backgroundColor: '#2D2D44', color: '#9B9BB0' }}>Fetch from ExerciseDB</button>
          </div>

          {loading ? <p style={{ color: '#6B6B8A' }}>Loading...</p> : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              {exercises.length === 0 && <p style={{ color: '#6B6B8A' }}>No exercises found. Try seeding the database first.</p>}
              {exercises.map(ex => (
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <p style={{ color: '#6B6B8A', fontSize: '0.75rem' }}>
                        {ex.type}{ex.family_name ? ` - ${ex.family_name}` : ''}
                      </p>
                    </div>
                    <button onClick={() => addToPool(ex)} style={{ ...btnStyle, backgroundColor: '#5B4FE8', color: '#fff', fontSize: '0.75rem' }}>
                      + Pool
                    </button>
                  </div>
                </div>
              ...
              {meta.total > 20 && (
                <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'center', marginTop: '1rem' }}>
                  ...
                  <span style={{ color: '#6B6B8A', fontSize: '0.875rem', alignSelf: 'center' }}>Page {meta.page}</span>
      ...
      {tab === 'pool' && (
        <>
          {loading ? <p style={{ color: '#6B6B8A' }}>Loading...</p> : pool.length === 0 ? (
            <p style={{ color: '#6B6B8A' }}>Your pool is empty. Browse exercises and add them to your pool for programme generation.</p>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              {pool.map(p => (
                <div key={p.id} style={cardStyle}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      ...
                      <p style={{ color: '#6B6B8A', fontSize: '0.75rem' }}>{p.type}</p>
                    </div>
                    <button onClick={() => removeFromPool(p.id)} style={{ ...btnStyle, color: '#F97316', fontSize: '0.75rem' }}>Remove</button>
                  </div>
                </div>
              ))}
            </div>
      ...
      {tab === 'seed' && (
        <div style={cardStyle}>
          <p style={{ marginBottom: '1rem' }}>
            Seed the exercise database from ExerciseDB (AscendAPI). This fetches all available exercises
            and caches them locally so you can browse and add them to your pool.
          </p>
          <button onClick={seedExercises} style={{ ...btnStyle, backgroundColor: '#5B4FE8', color: '#fff' }}>
            Seed All Exercises
          </button>
          {seedStatus && <p style={{ color: '#4FE8A8', marginTop: '1rem', fontSize: '0.875rem' }}>{seedStatus}</p>}
        </div>
      )}
    </Main>
  );
